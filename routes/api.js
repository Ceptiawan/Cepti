...
router.get('/addapikey', (req, res, next) => {
    ...
    if (apikey != 'Ganteng') return res.json(loghandler.invalidKey)
...
router.get('/cewe/vietnam', async (req, res, next) => {
        var apikeyInput = req.query.apikey
	if(!apikeyInput) return res.json(loghandler.notparam)	
	if (apikeyInput != 'Ganteng')  return res.json(loghandler.invalidKey)
...
router.get("/textmakercoffy", async(req, res, next) => {
